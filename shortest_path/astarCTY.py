...
class Heap:
    ...
    def downheap(self, idx):
        while True:
            ...
            right = (2*idx + 2)*3
            idx=idx*3
            if not (left < len(self.data) and right < len(self.data)):
                break
            ...
            if self.data[small] < self.data[idx]:  # 左节点存在
                self.swap(small, idx)
                idx=int(small/3)
            else:
                break
...
def a_starCTY(raster: np.ndarray, start_p, end_p, walk_type,fast_mode=True):
    ...
    # 用cost来索引最小的点，代替堆结构
    open_cost=Heap([est_total,start_p[0],start_p[1]])
    ...
    while True:
        # 索引cost字典得到最小的点
        # 从HEAP类里传出来的元组不能用于一些操作。用now_p_p来作为可使用的点
        now_p = open_cost.get_min()
        open_cost.remove_min()

        if now_p not in open_dict:
            continue
        now_pdata=open_dict[now_p]
        value=now_pdata.min_dist
        now_p=(now_pdata.x,now_pdata.y)
        open_dict.pop(now_p)
        ...
        for k, v in nearby:
            ...
            if k not in open_dict or open_dict[k].min_dist > min_dist:
                ...
                open_cost.add(pdata.total_dist)
                open_cost.add(k[0])
                open_cost.add(k[1])
                open_dict[k] = pdata
